...
void mutation (groupe *population) {
	...
	for(int i = 0; i < NBPOPULATION; i++){
	       	for(int j = 0; j < NBGENE/2; j++){
			proba = rand() % 200; //on prend aléatoirement un nombre pour pouvoir effecter des mutations de manière aléatoire avec une probabilité TAUX_MU
                        if(proba  < TAUX_MUT )
				population->membres[i].gene[j] = (char)rand(); //on change la valeur d'un gène aléatoirement pour chaque membress de la  population
                	}
        	}
	}		
